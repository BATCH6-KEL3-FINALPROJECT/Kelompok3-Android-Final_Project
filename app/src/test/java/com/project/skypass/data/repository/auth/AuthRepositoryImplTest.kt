...
class AuthRepositoryImplTest {
    ...
    @Test
    fun `doLogin no internet connection`() =
        runBlocking {
            val email = "email@example.com"
            val password = "password"
            val loginRequestResponse = LoginRequestResponse(email, password)
...
//                    )
    ...
    @Test
    fun `doLogin http exception`() =
        runBlocking {
            val email = "email@example.com"
            val password = "password"
            val loginRequestResponse = LoginRequestResponse(email, password)
            coEvery { ds.doLogin(loginRequestResponse) } throws
                ErrorInterceptor.HttpException(
                    404,
                    "Error",
                )
...
//                    assertEquals("java.lang.Exception: Error", result.exception?.message)
    ...
    @Test
    fun `doRegister no internet connection`() =
        runBlocking {
            val email = "email@example.com"
            val password = "password"
            val name = "name"
            ...
            coEvery {
                ds.doRegister(registerRequestResponse)
            } throws ErrorInterceptor.NoInternetException("java.lang.Exception: No internet connection")
