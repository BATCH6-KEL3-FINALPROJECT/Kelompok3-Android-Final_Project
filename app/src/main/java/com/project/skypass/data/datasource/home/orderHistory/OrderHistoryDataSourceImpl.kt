...
class OrderHistoryDataSourceImpl(private val dao: OrderHistoryDao) : OrderHistoryDataSource {
    ...
    override suspend fun insertOrderHistory(item: OrderHistoryEntity): Long = dao.insertOrderHistory(item)

    override suspend fun deleteOrderHistory(item: OrderHistoryEntity): Int = dao.deleteOrderHistory(item)
